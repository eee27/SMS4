...
/**
 * @author ST-ZHQ
 * @ClassName: SMS4Android
 * @Description: SMS4加密算法
 * @date 2017-9-22 上午09:44:44
 */
public class SMS4 {
    ...
    static {
        try {
            //stemsoft12345678  7ujm^YHN5tgb$RFV(生产)
            globalKey = "stemsoft12345678".getBytes("utf-8");
    ...
    private byte[] Sbox = {(byte) 0xd7, (byte) 0x90, (byte) 0xe9, (byte) 0xfe,
     ...
     * @Description: 加密
     * @author ST-ZHQ
     * @date 2017-9-22 上午09:45:25
     */
    public static byte[] encodeSMS4(String plaintext) throws UnsupportedEncodingException {
        key = globalKey;
        ...
        return SMS4.encodeSMS4(plaintext.getBytes("utf-8"));
    }

    public static String encodeSMS4toString(String plaintext) {
        key = globalKey;
        try {
            ...
            byte[] result = SMS4.encodeSMS4(plaintext.getBytes("utf-8"));
     ...
     * @param ciphertext
     * @return
     * @Title: decodeSMS4toString
     * @Description: 解密
     * @author ST-ZHQ
     * @date 2017-9-22 上午09:45:41
     */
    ...
    public static String decodeSMS4toString(String cipherStr) {
        key = globalKey;
        try {
            byte[] ciphertext = toByte(cipherStr);
            byte[] plaintext = new byte[ciphertext.length];
            plaintext = decodeSMS4(ciphertext);
            int i;
            for (i = 0; i < plaintext.length; i++) {
     ...
     * @param plaintext
     * @return
     * @Title: encodeSMS4toStringWithMD5Time
     * @Description: 时间作为key加密
     * @author ST-LBO
     * @date 2018-2-5 上午09:45:41
     */
    public static String encodeSMS4toStringWithMD5Time(String plaintext) {
        String md5 = makeTimeMD5();
        try {
            key = md5.getBytes("utf-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        try {
            ...
            byte[] result = SMS4.encodeSMS4(plaintext.getBytes("utf-8"));
            //return md5 + ":" + DatatypeConverter.printHexBinary(result);
    ...
    /**
     * @param md5Time,cipherStr
     * @return
     * @Title: decodeSMS4toStringWithMD5Time
     * @Description: 时间作为key解密
     * @author ST-LBO
     * @date 2018-2-5 上午09:45:41
     */
    public static String decodeSMS4toStringWithMD5Time(String md5Time, String cipherStr) {
        try {
            key = md5Time.getBytes("utf-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        try {
    ...
    public static String makeTimeMD5() {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMddHHmmss");
        String dateStr = dateFormat.format(new Date());

        try {
            MessageDigest md = MessageDigest.getInstance("MD5");
            md.update(dateStr.getBytes("utf-8"));
    ...
    public static String bcd2asc(byte[] b) {
        String hs = "";
        String stmp = "";
